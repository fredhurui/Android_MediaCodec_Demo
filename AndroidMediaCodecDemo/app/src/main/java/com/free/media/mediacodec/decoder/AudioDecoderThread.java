...
public class AudioDecoderThread {
    ...
    public void startPlay(String path) {
        ...
        for (int i = 0; i < mExtractor.getTrackCount(); i++) {
            ...
            if (mime.startsWith("audio/")) {
                ...
                for (int k = 0; k < csd.capacity(); ++k) {
                    MyLogger.e("TAG", "csd : " + csd.array()[k]);
                }
                mSampleRate = format.getInteger(MediaFormat.KEY_SAMPLE_RATE);
                channel = format.getInteger(MediaFormat.KEY_CHANNEL_COUNT);
                MyLogger.d("TAG", "mSampleRate : " + mSampleRate);
                MyLogger.d("TAG", "channel : " + channel);
                break;
            }
        }
        ...
        if (mDecoder == null) {
            MyLogger.e("DecodeActivity", "Can't find video info!");
            return;
        }
    ...
    private MediaFormat makeAACCodecSpecificData(int audioProfile, int sampleRate, int channelConfig) {
        ...
        for (int i = 0; i < samplingFreq.length; ++i) {
            if (samplingFreq[i] == sampleRate) {
                MyLogger.d("TAG", "kSamplingFreq " + samplingFreq[i] + " i : " + i);
                sampleIndex = i;
            }
        }
    ...
    public void AACDecoderAndPlay() {
        ...
        while (!eosReceived) {
            ...
            if (inIndex >= 0) {
                ...
                MyLogger.d("DecodeActivity", "InputBuffer sampleSize:" + sampleSize);
                for(int i = 0; i < sampleSize; i++) {
                    MyLogger.d("DecodeActivity", "InputBuffer [" + i + "]:" + buffer.get(i));
                }
                if (sampleSize < 0) {
                    // We shouldn't stop the playback at this point, just pass the EOS
                    // flag to mDecoder, we will get it again from the
                    // dequeueOutputBuffer
                    MyLogger.d("DecodeActivity", "InputBuffer BUFFER_FLAG_END_OF_STREAM");
                ...
                switch (outIndex) {
                    case MediaCodec.INFO_OUTPUT_BUFFERS_CHANGED:
                        MyLogger.d("DecodeActivity", "INFO_OUTPUT_BUFFERS_CHANGED");
                        outputBuffers = mDecoder.getOutputBuffers();
                        break;

                    case MediaCodec.INFO_OUTPUT_FORMAT_CHANGED:
                        MediaFormat format = mDecoder.getOutputFormat();
                        MyLogger.d("DecodeActivity", "New format " + format);
                        audioTrack.setPlaybackRate(format.getInteger(MediaFormat.KEY_SAMPLE_RATE));

                        break;
                    case MediaCodec.INFO_TRY_AGAIN_LATER:
                        MyLogger.d("DecodeActivity", "dequeueOutputBuffer timed out!");
                        break;

                    default:
                        ByteBuffer outBuffer = outputBuffers[outIndex];
                        MyLogger.v("DecodeActivity", "We can't use this buffer but render it due to the API limit, " + outBuffer);
                ...
                if ((info.flags & MediaCodec.BUFFER_FLAG_END_OF_STREAM) != 0) {
                    MyLogger.d("DecodeActivity", "OutputBuffer BUFFER_FLAG_END_OF_STREAM");
                    break;
                }
            }
        }
